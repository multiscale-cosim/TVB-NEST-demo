...
from nest_elephant_tvb.transformation.simulator_IO.Neurolib_IO import ConsumerNeurolibData
...
if __name__ == "__main__":
    ...
    logger = create_logger(path, 'neurolib_to_nest_' + str(id_transformer), level_log)
    ...
    while not os.path.exists(path + '/nest/spike_generator.txt.unlock'):
        ...
        logger.info(path + '/nest/spike_generator.txt.unlock')
        time.sleep(1)
    ...
    if MPI.COMM_WORLD.Get_size() == 3:  # MPI internal communication
        if rank == 0:  # communication with Nest
            send_data_to_Nest = ProducerDataNest(
                ...
                'neurolib_to_nest_sender' + str(id_first_spike_detector), path, level_log,
            ...
            transform_rate_to_spike = TransformationRateSpike(
                ...
                'neurolib_to_nest_transform' + str(id_first_spike_detector), path, level_log,
        ...
        path_to_files_receive = [path + "/transformation/receive_from_neurolib/" + str(id_proxy[id_transformer]) + ".txt"]
